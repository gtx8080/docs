...
## 在 思明智算中心 上使用 OpenClaw

ClawOS v0.1提供面向企业级的 OpenClaw 托管运行平台。无需自行配置环境或手动接入模型，5 分钟内即可让你的 AI 数字员工上岗。
...
!!! note
    ...
    简历属于敏感个人信息（PII）。私有化部署确保数据不出内网，禁止将简历传送至外部公有云 API。
...
| 安全沙箱隔离 | 每只小龙虾独立运行在容器安全沙箱中，基于 **zestU** 内核级隔离技术。即使实例被劫持或发生故障，影响被严格限制在沙箱内，无法渗透内网或篡改宿主机文件。 |
| 数据持久化，记忆永不丢失 | `~/.openclaw` 目录全量持久化存储在平台存储系统中。无论重启 OpenClaw、暂停还是释放实例，对话历史、配置与记忆均不会丢失。 |
| 模型调用安全可控 | 所有模型调用统一经过平台 AI 网关与安全策略，不直接裸连公网。你的数据和对话全程使用平台内的大模型服务，敏感信息受到保障。 |
| 后台完全可操作 | 提供 [SSH 登录和 JupyterLab](quickstart.md#_4) 两种方式访问小龙虾后台，支持完整的 CLI 操作，满足进阶用户的调试与定制需求。 |
